...
class ProductoService:
    ...
    def descontar_stock(self, producto_id: str, cantidad: int):
        ...
        # Si el stock llega a 0, eliminamos el producto del inventario
        if producto.stock == 0:
            deleted = self.repository.eliminar(producto_id)
            if deleted:
                return {"deleted": True, "message": "Producto eliminado porque el stock llegó a 0"}
            else:
                # Si no se pudo eliminar por alguna razón, devolvemos el objeto actualizado
                return self.repository.obtener(producto_id)
